...
class BatchKSSFramework:
    ...
    def step(self):
        ...
        for idx, env in enumerate(self.envs):
            exercise, correct = env.step(exercise=action.tolist()[idx])
    ...
    def episode_loop(self, max_steps_per_episode, desc=''):
        ...
        for idx, env in enumerate(self.envs):
            ...
            if reward > self.max_reward:
                ...
                if self.agent.name == 'CSEAL':
                    ...
                    ndarray.save('best_agent/KSS/{}.parmas'.format(self.agent.name), arg_dict)
                else:
                    torch.save(self.agent, 'best_agent/KSS/{}.pt'.format(self.agent.name))
            #
        ...
        # 四、更新环境和agent
        self.agent.episode_refresh(batch_reward)
    ...
    def batch_train(self, agent):
        ...
        if self.agent.name == 'CSEAL':
            ...
            best_agent = torch.load('best_agent/KSS/{}.pt'.format(self.agent.name), map_location=self.agent.device)

        return self.agent, best_agent, self.max_reward

    def batch_test(self, agent, test_times=100):
        self.agent = agent
        ability_list = []  # test_times(100)个元素，每个元素是一个self.max_steps(30)个元素的列表，其中每个元素记录每一时间步时学生的能力（一个10个元素的列表）
        sum_ability_list = []  # test_times(100)个元素，每个元素是一个self.max_steps(30)个元素的列表，其中每个元素记录每一时间步时学生的能力总和（一个数）
        sum_ability = 0
