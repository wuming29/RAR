...
class Env(object):
    ...
    def __call__(self, exercises):
        ...
        for exercise in exercises:
            ...
            v = self.state_transform(exercise, correct)
            a_mastety = sum(self._state)
            reward = a_mastety - p_mastery - 0.25
        return exercises_record, reward, v
            ...
            if random.random() < ORDER_RATIO:  # ORDER_RATIO = 1
                while cnt < step:
                    ...
                    if exercises_record and exercises_record[-1][1] == 1 and len(
                        ...
                        for _ in range(1):
                            # 再做一遍上一道题，并记录
                            exercises_record.append(self.step(exercises_record[-1][0], interactive=True)[0])
                        # 记录node，让他再做一遍
                    ...
                    exercises_record.append(self.step(node, interactive=True)[0])
            else:
                while cnt < step:
                    cnt += 1
                    if random.random() < 0.9:
                        ...
                        node = random.randint(0, len(self.topo_order) - 1)
                    exercises_record.append(self.step(node)[0])
        ...
        if random.random() < ORDER_RATIO:
            while cnt < step:
                ...
                if exercises_record and exercises_record[-1][1] == 1 and len(
                        set([e[0] for e in exercises_record[-3:]])) > 1:
                    for _ in range(1):
                        exercises_record.append(self.step(exercises_record[-1][0])[0])
                    node = exercises_record[-1][0]
                elif exercises_record and exercises_record[-1][1] == 0 and random.random() < 0.7:
                    node = exercises_record[-1][0]
                elif random.random() < 0.9:
