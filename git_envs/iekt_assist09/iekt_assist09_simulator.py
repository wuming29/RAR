...
class dkt(nn.Module): 
    def __init__(self, args):
        ...
        self.predictor = modules.funcs(args.n_layer, args.dim, args.problem_number, args.dropout) #args.dim + 
...
class iekt_c(nn.Module): 
    '''
    原始版本的iekt需要ground truth，但实际上当作模拟器的时候是没有ground truth的。
    这里将ground truth去掉
    '''
    def __init__(self, args):
        super().__init__()
        ...
        self.ques_num = 15003
        with open('data/assist09/problem_skills_relation.pkl', 'rb') as f:
            ques_concept_mapping = pickle.load(f)
            self.ques_concept_table = torch.tensor(list(ques_concept_mapping.values()), device=self.device)
    ...
    def step(self, h, batch_problem_id):
        with torch.no_grad():
            ...
            # ques_h: batch_size*(3*node_dim(64))
            # self.get_ques_representation返回128维问题表示，拼接h64维，共192维。
            ques_h = torch.cat([
    ...
    def total_probability(self):
        # (batch_size*ques_num)*node_dim(64)
        test_h = self.h.repeat(self.ques_num, 1)

        # (batch_size*ques_num)向量，[1,1,1,...,1, 2,2,...,2, 3,3,...,2163,2163,...,2163]
        ...
        # 长度为ques_num的tuple，每个元素是测试的batch个学生回答当前问题的回答正确率
        batch_prob = torch.split(prob.reshape(-1), self.batch_size)
        # ques_num*batch_size矩
        batch_prob = torch.stack(batch_prob, dim=0)

        # batch_size*ques_num矩
        total_probability = batch_prob.T

        return total_probability

    def test_target_score(self, target):  # target:batch_size * target_num矩
        with torch.no_grad():
            target_num = target.size()[1]

            # (batch_size*target_num) * node_dim(64)。每相邻target_num个向量相同，打包为一组，共batch_size组。
            test_h = self.h.repeat_interleave(target_num, dim=0)

            # (batch_size*target_num)向量。[学生1target1, 学生1target2, ..., 学生2target1, ...]
            ...
            # 长度为batch_size的tuple，每个元素是测试的target个问题的回答正确率
            batch_prob = torch.split(prob.reshape(-1), target_num)
            # batch_size*target_num矩
            batch_prob = torch.stack(batch_prob, dim=0)
            # batch_size长度向量
    ...
    def test_massive_target_score(self, massive_target, test_times=10):  # target:batch_size * target_num矩
        with torch.no_grad():
