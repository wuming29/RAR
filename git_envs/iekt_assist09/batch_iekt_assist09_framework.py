...
class BatchIEKTassist09Simulator:
    ...
    def batch_train(self, batch_size, agent):
                ...
                for i_episode in range(int(self.episodes / self.epoch_num)):
                    ...
                    if this_reward > max_reward:
                        ...
                        if self.agent.name == 'CSEAL':
                            ...
                            ndarray.save('save_model/IEKTassist09/{}.parmas'.format(self.agent.name), arg_dict)
                        # elif self.agent.name == 'SRC':
                        #     from mindspore import save_checkpoint
                        #     save_checkpoint(self.agent.model, 'save_model/IEKTassist09/{}.ckpt'.format(self.agent.name))
                        else:
                            torch.save(self.agent, 'save_model/IEKTassist09/{}.pt'.format(self.agent.name))
        ...
        #     save_model = None
            ...
            save_model = torch.load('save_model/IEKTassist09/{}.pt'.format(self.agent.name), map_location=self.agent.device)
    ...
    def train(self, agent):
        ...
        plt.savefig('experiment_record/plot/{} on iekt_assist09.png'.format(self.agent.name))

        with open('experiment_record/train_record/iekt_assist09/episode_return_list/{}.pkl'.format(self.agent.name), 'wb') as f:
            pickle.dump(return_list, f)

        try:
            torch.save(self.agent, 'experiment_record/trained_agent_model/iekt_assist09/{}.pt'.format(self.agent.name))
        except TypeError:
            pass
