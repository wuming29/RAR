...
class BatchIEKTjunyiSimulator:
    ...
    def batch_train(self, batch_size, agent):
                ...
                for i_episode in range(int(self.episodes / self.epoch_num)):
                    ...
                    if this_reward > max_reward:
                        ...
                        if self.agent.name == 'CSEAL':
                            ...
                            ndarray.save('save_model/IEKTjunyi/{}.parmas'.format(self.agent.name), arg_dict)

                        else:
                            torch.save(self.agent, 'save_model/IEKTjunyi/{}.pt'.format(self.agent.name))
        ...
        if self.agent.name == 'CSEAL':
            ...
            saved_model = torch.load('save_model/IEKTjunyi/{}.pt'.format(self.agent.name), map_location=self.agent.device)
    ...
    def train(self, agent):
        ...
        plt.savefig('experiment_record/plot/{} on DKT_junyi.png'.format(self.agent.name))

        with open('experiment_record/train_record/DKT_junyi/episode_return_list/{}.pkl'.format(self.agent.name), 'wb') as f:
            pickle.dump(return_list, f)

        try:
            torch.save(self.agent, 'experiment_record/trained_agent_model/DKT_junyi/{}.pt'.format(self.agent.name))
        except TypeError:
            pass
