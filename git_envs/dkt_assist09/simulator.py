...
class dkt(nn.Module): 
    ...
    def forward(self, inputs):
        ...
        probs = []
        total_pre = []
        data_len = len(inputs[0])  # 该batch下的学生数（设定为200）
        h = torch.zeros(data_len, self.node_dim).to(self.device)
        # for i in range(0, self.seq_length):
        for i in range(0, data_len):
            h, prob, total_pre = self.cell(h, inputs[1][i])  # inputs[1][i]是第i个序列节点的200位学生的信息
        ...
        # constrain_pdkt = 0
        for i in range(0, data_len):
            this_prob = prob_tensor[i][0 : seq_num[i]]  # seq_num[i]：第i个学生的答题序列长度
