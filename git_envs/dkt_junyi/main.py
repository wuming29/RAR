...
parser.add_argument('--data_dir',       type=str,   default='data/assist09/') #assistment2009-2010
...
parser.add_argument('--model',          type=str,   default='dkt',   help='run model')
...
log.getLogger().addHandler(log.FileHandler(os.path.join(args.run_dir, 'mod_log.txt'), mode='w'))
...
if __name__ == '__main__':
    loaders = preprocess()
    Model = getattr(dkt_assist09_simulator, args.model)
    if args.checkpoint_path != 'none':
        model = torch.load(args.checkpoint_path, map_location = torch.device(args.device))
    else:
        model = Model(args).to(args.device)
    log.info(str(vars(args)))
    # model = torch.load('nor_dktmodel.pt', map_location=torch.device(args.device))
