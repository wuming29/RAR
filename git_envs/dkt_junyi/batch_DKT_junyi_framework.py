...
class BatchDKTjunyiSimulator:
    ...
    def batch_train(self, batch_size, agent):
                ...
                for i_episode in range(int(self.episodes / self.epoch_num)):
                    ...
                    if this_reward > max_reward:
                        ...
                        if self.agent.name == 'CSEAL':
                            ...
                            ndarray.save('save_model/DKTJU/{}.parmas'.format(self.agent.name), arg_dict)
                        else:
                            torch.save(self.agent, 'save_model/DKTJU/{}.pt'.format(self.agent.name))
        ...
        if self.agent.name == 'CSEAL':
            ...
            save_model = torch.load('save_model/DKTJU/{}.pt'.format(self.agent.name), map_location=self.agent.device)
